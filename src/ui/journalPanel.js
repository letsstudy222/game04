...
import { ACHIEVEMENTS } from '../core/journal.js';
...
export function renderJournal(el, journal) {
  ...
  const achRows = Object.entries(ACHIEVEMENTS).map(([id, a]) => {
    const got = journal.achievements.has(id);
    return `<div class="j-ach ${got ? 'met' : ''}" title="${a.desc}">
      <span class="j-ach-icon">${got ? '🏆' : '🔒'}</span>
      <span class="j-ach-name">${got ? a.viet : a.desc}</span>
    </div>`;
  }).join('');
